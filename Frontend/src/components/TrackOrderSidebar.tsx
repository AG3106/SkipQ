...
import { Package, CheckCircle, Clock, X, ChefHat, Phone, Loader2 } from "lucide-react";
...
export default function TrackOrderSidebar() {
  ...
  const statusMap: Record<string, { icon: React.ReactNode; text: string; color: string }> = {
    ...
    READY: {
      icon: <Package className="w-5 h-5 text-[#D4725C]" />,
      text: "Ready",
      color: "bg-[#D4725C]/10 dark:bg-[#D4725C]/20 text-[#D4725C] dark:text-orange-400 border-[#D4725C]/20 dark:border-[#D4725C]/30",
    },
    ACCEPTED: {
  ...
  // Show only recent/active orders (not cancelled/refunded)
  const activeStatuses = ["PENDING", "ACCEPTED", "READY", "COMPLETED"];
                    ...
                    >
                      ...
                      <div className="flex items-start justify-between mb-4">
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-lg font-black text-[#D4725C]">₹{parseFloat(order.totalPrice).toFixed(0)}</p>
                        </div>
                      </div>
                      ...
                      {order.status !== "COMPLETED" && (
                        <div className="mb-5">
                          <div className="flex justify-between text-[10px] uppercase font-bold text-gray-400 dark:text-gray-500 mb-2 tracking-wider">
                            <span className={order.status === "ACCEPTED" || order.status === "READY" || order.status === "PENDING" ? "text-[#D4725C]" : ""}>Placed</span>
                            <span className={order.status === "ACCEPTED" || order.status === "READY" ? "text-[#D4725C]" : ""}>Cooking</span>
                      ...
                      <div className="bg-white/50 dark:bg-gray-900/50 rounded-xl p-3 mb-4 border border-gray-50 dark:border-gray-800">
                        ...
                        <ul className="space-y-1.5">
                          {order.items.map((item) => (
                            <li key={item.id} className="text-sm text-gray-700 dark:text-gray-300 flex items-center gap-2 font-medium">
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-gray-100/50 dark:border-gray-800/50">
                        <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                          ...
                          <span>
                            {order.status === "COMPLETED" ? "Completed" : "Ordered"}: <span className="text-gray-900 dark:text-white font-bold">{formatTime(order.bookTime)}</span>
                          </span>
                        </div>
                        <span className="text-xs font-medium text-gray-400 dark:text-gray-500">{order.canteenName}</span>
                      </div>
                    </motion.div>
                  );
                })
